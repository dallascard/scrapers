...
def main():
    usage = "%prog\n" \
            "Scrape statements and remarks from the American Presidency Project"
    parser = OptionParser(usage=usage)
    parser.add_option('--outdir', type=str, default='data/app/statements/',
                      help='Output directory): default=%default')
    parser.add_option('--pages', type=int, default=882,
    ...
    for page in range(pages):
        if page == 0:
            url = 'https://www.presidency.ucsb.edu/documents/app-categories/written-statements/presidential/statements'
            index_file = os.path.join(outdir, 'index.html')
        else:
            url = 'https://www.presidency.ucsb.edu/documents/app-categories/written-statements/presidential/statements?page=' + str(page)
    ...
    with open(os.path.join(outdir, 'statements.all.jsonlist'), 'w') as fo:
