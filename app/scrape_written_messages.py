...
def main():
    ...
    parser.add_option('--pages', type=int, default=1230,
    ...
    for page in range(pages):
        if page == 0:
            url = 'https://www.presidency.ucsb.edu/documents/app-categories/presidential/written-messages'
            index_file = os.path.join(outdir, 'index.html')
        else:
            url = 'https://www.presidency.ucsb.edu/documents/app-categories/presidential/written-messages?page=' + str(page)
    ...
    print("Saving {:d} messages".format(len(outlines)))
    with open(os.path.join(outdir, 'written_messages.all.jsonlist'), 'w') as fo:
